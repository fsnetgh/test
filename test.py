...
def jiaoYiCelue(TRiShouPanJia,TRiMA20Zhi,T_1RiShouPanJia,T_1RiMA20Zhi,T_2RiShouPanJia,T_2RiMA20Zhi):
    ...
    return jieGuo
#------------------------------------------------------------
def zhiXingJiaoYi(TRiJiaoYiCeLue,T_1RiCangWeiShuLiang):
    '''
    当t日策略为“买入”并且t-1日仓位数量=0时，t日执行买入操作既持仓数+1
    当t日策略为“买入”并且t-1日仓位数量<>0时，t日报告错误
    当t日策略为“卖出”并且t-1日仓位数量>0时，t日执行卖出操作既持仓数-1
    当t日策略为“卖出”并且t-1日仓位数量<=0时，t日报告错误
    当t日策略为“持有”并且t-1日仓位数量>0时，t日执行持有操作既持仓数不变
    当t日策略为“持有”并且t-1日仓位数量<=0时，t日报告错误
    当t日策略为“空仓”并且t-1日仓位数量=0时，t日执行空仓操作既持仓数不变
...
for chuShiZhi in range(24,400,1):
    ...
    gongZuoBiao['n' + str(chuShiZhi)].value = zhiXingJiaoYi(gongZuoBiao['m'+str(chuShiZhi)].value,gongZuoBiao['n'+str(chuShiZhi-1)].value)
